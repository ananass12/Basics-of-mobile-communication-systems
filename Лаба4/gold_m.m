...
fprintf('\nКорреляция: ');
for tau = 0:N-1
    gold2_shifted = circshift(gold2, [0, tau]);
    R = norm_corr(gold1, gold2_shifted);
    fprintf('%.3f ', R);
end
fprintf('\n\n');

ac_vals = auto_corr_xcorr(gold1, N-1);
...
figure;
lags = 0:N-1;
stem(lags, ac_vals, 'filled', 'LineWidth', 1.2);
